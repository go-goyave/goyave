...
func loadGoyaveRouter(routes []route) *Router {
	...
	for _, r := range routes {
		router.Route(r.method, r.path, goyaveHandler, nil)
	}
...
func Benchmark_Validate(b *testing.B) {
	...
	w := httptest.NewRecorder()
	r, _ := http.NewRequest("POST", "/test/validate", bytes.NewBuffer(body))

	b.ReportAllocs()
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		router.requestHandler(w, r, goyaveHandler, rule)
	}
}
