...
func (r *Route) Name(name string) *Route {
	if r.name != "" {
		panic(errwrap.NewSkip("route name is already set", 3))
	}

	if _, ok := r.parent.namedRoutes[name]; ok {
		panic(errwrap.NewSkip(fmt.Errorf("route %q already exists", name), 3))
	}
...
func (r *Route) BuildURI(parameters ...string) string {
	...
	if len(parameters) != len(fullParameters) {
		panic(errwrap.Errorf("BuildURI: route has %d parameters, %d given", len(fullParameters), len(parameters)))
	}
