...
func (suite *CommonTestSuite) TestCommonFormatter() {
	...
	suite.Equal("192.0.2.1 - - [23/Mar/2020:13:58:26 +0000] \"GET \"/log\" HTTP/1.1\" 204 5", CommonLogFormatter(ts, response, request, 5))

	request = suite.CreateTestRequest(httptest.NewRequest("GET", "http://user@localhost/log", nil))
	suite.Equal("192.0.2.1 - user [23/Mar/2020:13:58:26 +0000] \"GET \"http://user@localhost/log\" HTTP/1.1\" 204 5", CommonLogFormatter(ts, response, request, 5))

	// Invalid ipv6 URL
	request = suite.CreateTestRequest(httptest.NewRequest("GET", "/log", nil))
	request.Request().RemoteAddr = "[::1"
	suite.Equal("[::1 - - [23/Mar/2020:13:58:26 +0000] \"GET \"/log\" HTTP/1.1\" 204 5", CommonLogFormatter(ts, response, request, 5))
	...
	suite.Equal("192.0.2.1 - - [23/Mar/2020:13:58:26 +0000] \"CONNECT \"example.com\" HTTP/2.0\" 204 5", CommonLogFormatter(ts, response, request, 5))
...
func (suite *CommonTestSuite) TestCombinedFormatter() {
	...
	suite.Equal("192.0.2.1 - - [23/Mar/2020:13:58:26 +0000] \"GET \"/log\" HTTP/1.1\" 204 5 \""+referrer+"\" \""+userAgent+"\"", CombinedLogFormatter(ts, response, request, 5))
}
