...
var (
	ErrSessionEnded     = errors.New("testutil.Session: session already ended")
	ErrEndRootSession   = errors.New("testutil.Session: cannot commit/rollback root session")
	ErrChildRunning     = errors.New("testutil.Session: cannot commit/rollback if a child session is still running")
	ErrNotParentContext = errors.New("testutil.Session: cannot create a child session with an unrelated context. Parent context should be the context or a child context of the parent session")
)
...
func (s *Session) Begin(ctx context.Context) (session.Session, error) {
	...
	if s.cancel != nil && parent != s {
		...
		return nil, errwrap.New(ErrNotParentContext)
	}
...
func (s *Session) Transaction(ctx context.Context, f func(context.Context) error) error {
	...
	err = errwrap.New(f(tx.Context()))
	if err != nil {
		rollbackErr := errwrap.New(tx.Rollback())
		return errwrap.New([]error{err, rollbackErr})
	}
	return errwrap.New(tx.Commit())
}
...
func (s *Session) Rollback() error {
	if s.cancel == nil {
		return errwrap.New(ErrEndRootSession)
	}
	if s.hasRunningChild() {
		return errwrap.New(ErrChildRunning)
	}
	swapped := s.status.CompareAndSwap(SessionCreated, SessionRolledBack)
	if !swapped {
		return errwrap.New(ErrSessionEnded)
	}
	s.cancel()
	return nil
}
...
func (s *Session) Commit() error {
	...
	if err := s.ctx.Err(); err != nil {
		if s.Status() != SessionCreated {
			return errwrap.New([]error{err, ErrSessionEnded})
		}
		return errwrap.New(err)
	}
	if s.hasRunningChild() {
