...
if [ $? -ne 0 ]; then
	...
	gotest -v -coverprofile=c.out -coverpkg=./... ./... ; go tool cover -html=c.out -o=coverage.html ; go tool cover -func=c.out | grep total ; rm c.out
else
	gotest -v -race -coverprofile=c.out -coverpkg=./... ./... ; go tool cover -html=c.out -o=coverage.html ; go tool cover -func=c.out | grep total ; rm c.out
fi
test_result=$?
